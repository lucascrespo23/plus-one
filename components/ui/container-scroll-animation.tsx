...
export const ContainerScroll = ({
  ...
  const rotateRaw = useTransform(scrollYProgress, [0, 1], [10, 0]);
  const scaleRaw = useTransform(scrollYProgress, [0, 1], scaleDimensions());
  const translateRaw = useTransform(scrollYProgress, [0, 1], [0, -100]);

  const rotate = useSpring(rotateRaw, { stiffness: 100, damping: 30 });
  const scale = useSpring(scaleRaw, { stiffness: 100, damping: 30 });
  const translate = useSpring(translateRaw, { stiffness: 100, damping: 30 });

  return (
    <div className="h-[50rem] md:h-[70rem] flex items-center justify-center relative p-2 md:p-20" ref={containerRef}>
...
export const Card = ({
  ...
  >
    <div className="h-full w-full overflow-hidden rounded-2xl bg-gray-100 md:rounded-2xl md:p-4 flex flex-col justify-center">
      {children}
    </div>
  </motion.div>
);
